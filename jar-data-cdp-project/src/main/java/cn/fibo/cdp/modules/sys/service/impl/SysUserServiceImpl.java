...
public class SysUserServiceImpl extends ServiceImpl<SysUserDao, SysUserEntity> implements SysUserService {
	...
	public PageUtils queryPage(Map<String, Object> params) {
		...
		QueryWrapper queryWrapper =new QueryWrapper<SysUserEntity>()
				.like(StringUtils.isNotBlank(username),"a.username", username)
				.eq(createUserId != null,"a.create_user_id", createUserId)
				.eq("a.del_flag", StatusConstants.del_flag_0).groupBy("a.user_id");
		IPage<SysUserEntity> page = baseMapper.selectUserList(new Query<SysUserEntity>().getPage(params),queryWrapper);
	...
	private void checkRole(SysUserEntity user){
		...
		if(!roleIdList.containsAll(user.getRoleIdList())){
			throw new CustomExceptionException("新增用户所选角色，不是本人创建");
		}
	}
}
