...
public class SysMenuController extends AbstractController {
	...
	@GetMapping("/nav")
	public R nav(){
		JSONArray menuList = sysMenuService.getUserMenuList(getUserId());
		Set<String> permissions = shiroService.getUserPermissions(getUserId());
		Map<String,Object> rtnJson =  new HashMap<>();
		rtnJson.put("menuList",menuList);
		rtnJson.put("permissions",permissions);
		return R.okDatas(rtnJson);
	}
...
//	@RequiresPermissions("sys:menu:delete")
	...
	private void verifyForm(SysMenuEntity menu){
		if(StringUtils.isBlank(menu.getName())){
			throw new CustomExceptionException("菜单名称不能为空");
		}

		if(menu.getParentId() == null){
			throw new CustomExceptionException("上级菜单不能为空");
		}

		//菜单
		if(menu.getType() == Constant.MenuType.MENU.getValue()){
			if(StringUtils.isBlank(menu.getUrl())){
				throw new CustomExceptionException("菜单URL不能为空");
			}
		}
		...
		if(menu.getType() == Constant.MenuType.CATALOG.getValue() ||
			...
			if(parentType != Constant.MenuType.CATALOG.getValue()){
				throw new CustomExceptionException("上级菜单只能为目录类型");
			}
			return ;
		}
		...
		if(menu.getType() == Constant.MenuType.BUTTON.getValue()){
			if(parentType != Constant.MenuType.MENU.getValue()){
				throw new CustomExceptionException("上级菜单只能为菜单类型");
			}
			return ;
		}
	}
