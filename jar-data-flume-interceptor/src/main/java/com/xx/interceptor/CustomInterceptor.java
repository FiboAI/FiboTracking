...
public class CustomInterceptor implements Interceptor {
    ...
    public Event intercept(Event event) {
        ...
        String method = "";
        try {
            String ip = eventBody.split(" -")[0];
            logger.info("源数据：" + eventBody+" \n");
           ...
           logger.info("解析后数据：" + preData+" \n");
            if (isIos){
                preData = java.net.URLDecoder.decode(preData, "utf-8");
                logger.info("IOS第一次转译后数据：" + preData+" \n");
                if (preData.contains("%")){
                    preData = java.net.URLDecoder.decode(preData, "utf-8");
                }
            }else {
            ...
            if(StringUtils.isNotBlank(project)){
                ...
                returnData.addProperty("ip",ip);
                returnData.addProperty("flume_time",System.currentTimeMillis());
