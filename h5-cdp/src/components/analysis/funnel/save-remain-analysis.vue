...
const confrimFunc = () => {
  // console.log('modelId.value:', modelId.value)
  let data = JSON.parse(JSON.stringify(funnel_analysis_data));

  if (funnel_analysis_data.overviewId !== undefined) {
    if (modelId.value) {
...
const init = (data: CalculateDataRequest) => {
  ...
  // delete data.selectPropertyParams
  // console.log(data);
